...
export const index = async (req: Request, res: Response): Promise<Response> => {
  ...
  const { data: requestUser } = await supabase.from("users").select("*").eq("id", id).maybeSingle();
...
export const show = async (req: Request, res: Response): Promise<Response> => {
  ...
  const { data: requestUser } = await supabase.from("users").select("*").eq("id", requestUserId).maybeSingle();
...
export const listPlan = async (req: Request, res: Response): Promise<Response> => {
  const { id } = req.params;
  const { companyId, companyUuid, profile } = req.user;

  if (profile === "admin" || companyId.toString() === id || companyUuid === id) {
    const resolvedId = (companyUuid === id) ? companyId : id;
    const company = await ShowPlanCompanyService(resolvedId);
    return res.status(200).json(company);
  } else {
    return res.status(400).json({ error: "Você não possui permissão para acessar este recurso!" });
  }
