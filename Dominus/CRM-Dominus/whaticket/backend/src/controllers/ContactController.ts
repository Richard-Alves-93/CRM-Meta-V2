      ...
      try {
        ...
        if (!tag) {
          const { data: newTag } = await supabase
            .from("tags")
            .insert({ name: tagName, company_id: companyId, color: "#A4CCCC", kanban: 0 })
            .select("*")
            .single();
          tag = newTag;
        }
        ...
        if (tag) {
          // ContactTag.findOrCreate equivalent
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const { data: findContact } = await supabase
    .from("contacts")
    .select("id")
    .eq("number", newContact.number.replace("-", "").replace(" ", ""))
    .eq("company_id", companyId)
    .maybeSingle();
  if (findContact) {
