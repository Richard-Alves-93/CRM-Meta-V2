  ...
  if (!companyUser) {
    ...
    try {
      const _email = {
        ...
        text: `Olá, ${name}!<br><br>
        Seu cadastro na empresa ${companyName} foi realizado com sucesso.<br><br>
        Confira os dados da sua conta:<br><br>Nome da empresa: ${companyName}<br>E-mail: ${email}<br>Senha: ${password}<br>Data de vencimento do período de teste: ${dateToClient(
          date
        )}`
      };
...
export const update = async (
  ...
  if (profile !== "admin" && String(userId) !== String(requestUserId)) {
    throw new AppError("ERR_NO_PERMISSION", 403);
  }
...
export const list = async (req: Request, res: Response): Promise<Response> => {
  ...
  const users = await SimpleListService({
    companyId: companyId ? String(companyId) : userCompanyId
  });
...
export const mediaUpload = async (
  ...
  if (profile !== "admin" && String(userId) !== String(requesterId)) {
    throw new AppError("ERR_NO_PERMISSION", 403);
  }
