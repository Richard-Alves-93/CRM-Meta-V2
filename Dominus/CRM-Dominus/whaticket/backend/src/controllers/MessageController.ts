...
export const addReaction = async (
  ...
  try {
    ...
    return res.status(statusCode).json({
      message: (error as any).message || "Erro ao adicionar reação"
    });
  }
};
...
export const index = async (
  ...
  const { ticketId } = req.params;
  const { pageNumber, selectedQueues: queueIdsStringified, searchParam } =
  ...
  // P4 FIX: Removido User.findByPk (Sequelize não funciona com UUID)
  // ListMessagesService agora é 100% Supabase e não precisa do user object
...
export const store = async (
  ...
  const { ticketId } = req.params;
  const { body, quotedMsg, vCard, isPrivate = "false", location }: MessageControllerData =
    req.body;
  ...
  try {
      ...
      await Promise.all(
        medias.map(async (media: Express.Multer.File, index) => {
          if (ticket.channel === "whatsapp") {
            const parsedQuotedMsg = typeof quotedMsg === 'string' ? JSON.parse(quotedMsg) : quotedMsg;
            const sentMedia = await SendWhatsAppMedia({
              ...
              isForwarded: false,
              quotedMsg: parsedQuotedMsg
            });

            if (isPrivate !== "true") {
              await verifyMediaMessage(sentMedia, ticket, ticket.contact, null, false, false, (await GetTicketWbot(ticket)));
            }
          }
    ...
    // P5 FIX: Propagar o statusCode real (403, 404, etc) vindo dos serviços Supabase
...
export const forwardMessage = async (
  ...
  // P5 FIX: Buscar usuário no Supabase
  ...
  const createTicket = await mutex.runExclusive(async () => {
    const result = await FindOrCreateTicketService(
      contact,
      ticket?.whatsapp || { id: ticket.whatsapp_id || ticket.whatsappId, company_id: companyId },
      0,
      ticket.companyId,
      ticket.queueId,
      requestUser!.id,
...
export const send = async (
  ...
  const messageData: MessageControllerData = req.body;
  const medias = req.files as Express.Multer.File[];
...
export const edit = async (
  ...
  const { messageId } = req.params;
  const { companyId } = req.user;
  const { body }: MessageControllerData = req.body;
