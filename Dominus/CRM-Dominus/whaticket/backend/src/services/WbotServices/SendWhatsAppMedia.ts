...
const SendWhatsAppMedia = async ({
  ...
  try {
    const wbot = await getWbot(ticket.whatsappId || ticket.whatsapp_id);
    const companyId = String(ticket.companyId || ticket.company_id);

    const pathMedia = media.path;
    const mimeType = media.mimetype || "";
    const ext = path.extname(media.originalname || pathMedia || "").toLowerCase();
    let options: AnyMessageContent | any;
    ...
    if (isPrivate === true) {
      ...
      await CreateMessageService({ messageData, companyId: ticket.companyId || ticket.company_id });

      // Retorna um stub apenas para satisfazer a assinatura (quem chama não usa esse retorno)
      return {} as unknown as WAMessage;
    }

    // Descobre o JID do contato do Supabase
    const { data: contactNumber } = await supabase
      .from("contacts")
      .select("number, remote_jid")
      .eq("id", ticket.contactId || ticket.contact_id)
      .maybeSingle();

    let number: string;

    if (
      contactNumber?.remote_jid &&
      contactNumber.remote_jid !== "" &&
      contactNumber.remote_jid.includes("@")
    ) {
      number = contactNumber.remote_jid;
    } else {
      number = `${contactNumber?.number}@${(ticket.isGroup || ticket.is_group) ? "g.us" : "s.whatsapp.net"}`;
    }

    // Envia
    const sentMessage = await wbot.sendMessage(number, { ...options }, { ...quotedOptions });

    await supabase
      .from("tickets")
      .update({
        last_message: body && body !== media.filename ? body : bodyMedia,
