...
const useStyles = makeStyles((theme) => ({
  ...
  messageLeft: {
    ...
    borderRadius: "18px", // Visual de pílula moderna
    borderTopLeftRadius: "4px", // Pequeno detalhe para indicar quem fala
    padding: "10px 14px",
  ...
  messageRight: {
    ...
    whiteSpace: "pre-wrap",
    backgroundColor: theme.mode === 'light' ? "#eff6ff" : "#2563eb",
    color: theme.mode === 'light' ? "#1e3a8a" : "#ffffff",
    alignSelf: "flex-end",
    borderRadius: "18px",
  ...
  messageRightPrivate: {
    ...
    padding: "8px 12px",
    boxShadow: theme.mode === 'light' ? "0 4px 6px -1px rgba(15, 23, 42, 0.05)" : "0 4px 6px -1px rgba(0, 0, 0, 0.3)",
    fontFamily: '"Inter", "Segoe UI", "Helvetica Neue", Arial, sans-serif',
  },
...
const MessagesList = ({
  ...
  }, [ticketId, ticketUuid, ticketDbId, selectedQueuesMessage, searchParam]);

  useEffect(() => {
    setLoading(true);
    const delayDebounceFn = setTimeout(() => {
      const fetchMessages = async () => {
        const targetId = ticketId || ticketUuid || ticketDbId;
        ...
        try {
          ...
          if (currentTicketId.current === ticketId || targetId === ticketUuid || targetId === ticketDbId) {
  ...
  }, [pageNumber, ticketId, ticketUuid, ticketDbId, selectedQueuesMessage, searchParam]);
  ...
  useEffect(() => {
    ...
    const connectEventMessagesList = () => {
      socket.emit("joinChatBox", `${targetId}`);
    }
    ...
    const isCurrentTicketEvent = (data) => {
      ...
      // Detectar o que estamos esperando (UUID ou ID numérico)
      const expectedUuid = isUUID(ticketUuid) ? ticketUuid : (isUUID(ticketId) ? ticketId : null);
      const expectedId = ticketDbId || (!isUUID(ticketId) && !isNaN(Number(ticketId)) ? Number(ticketId) : null);
    ...
    return () => {
      ...
      socket.emit("joinChatBoxLeave", `${targetId}`)
...
const renderMessageHeader = (classes, message) => {
  const senderName = message.fromMe ? getInternalSenderName(message) : message.contact?.name;

  if (!senderName) return null;

  return (
    <div className={classes.internalSenderHeader}>
        ...
        return (
          <React.Fragment key={message.id}>
            ...
            >
              ...
              {renderMessageHeader(classes, message)}
              {isYouTubeLink(message.body) && (
                <>
                  <YouTubePreview videoUrl={message.body} />
                </>
